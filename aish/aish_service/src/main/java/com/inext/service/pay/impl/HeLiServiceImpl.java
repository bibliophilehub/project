...
public class HeLiServiceImpl extends AbstractPayForAnother {
    ...
    public ApiServiceResult<PaymentChannelEnum> paymentApi(AssetBorrowOrder assetBorrowOrder) throws Exception {
        ...
        sPara.put("P3_customerNumber", Constants.BACK_CONFIG_PARAMS_MAP.get(BackConfigParams.HELIPAY).get("DF_MCHNTCD"));
        ...
        sPara.put("sign", RSA.sign(sb.toString(), RSA.getPrivateKey(Constants.BACK_CONFIG_PARAMS_MAP.get(BackConfigParams.HELIPAY).get("DF_SIGNKEY_TRANSFER"))));
        OutOrders orders = new OutOrders();
        String result = "";
        try {
            logger.info("代付参数 : {}", sPara.toString());
            result = OkHttpUtils.postForm(urlTransfer, sPara);
            logger.info("代付响应 : {}", result);
            JSONObject json = JSONObject.fromObject(result);
            if (json.getString("rt2_retCode").equals("0000")) {
                // 成功
                orders.setStatus(OutOrders.STATUS_SUC);
                return new ApiServiceResult("代付请求已接受").setExt(PaymentChannelEnum.HELIPAY);
            } else {
                // 流转状态 可忽略
            ...
            orders.setOrderType(OutOrders.TYPE_HELI);
