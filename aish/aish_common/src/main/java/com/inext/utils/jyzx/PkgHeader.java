...
public class PkgHeader {
    ...
    public Charset GetEncoding() {
        return this.encode.equals("02") ? Charset.forName("GBK") : Charset
                .forName("UTF-8");
    }
    ...
    public void parseFromString(String pkgStr) {
        ...
        String msgBody = "";
        try {
            msgBody = new String(new BASE64Decoder()
                    .decodeBuffer(postDataStr[6]), GetEncoding());
        ...
        String retMsg = "";
        try {
            retMsg = new String(new BASE64Decoder()
                    .decodeBuffer(postDataStr[8]), GetEncoding());
        } catch (IOException e) {
            e.printStackTrace();
    ...
    public String toPkgStr(Charset charset) {
        String msgBody = new BASE64Encoder().encode(this.getMsgBody().getBytes(
                charset));
        String retMsg = new BASE64Encoder().encode(this.getRetMsg().getBytes(
                charset));

        String result = this.getVersion() + "|" + this.getCustNo() + "|"
                + this.getEncode() + "|" + this.getTrxCode() + "|"
                + this.getEncryptType() + "|" + this.getMsgType() + "|"
                + msgBody + "|" + this.getRetCode() + "|" + retMsg + "|"
                + this.getSign();
    ...
    public String toString() {
        return "PkgHeader [custNo=" + custNo + ", encode=" + encode
                + ", encryptType=" + encryptType + ", msgBody=" + msgBody
                + ", msgType=" + msgType + ", retCode=" + retCode + ", retMsg="
                + retMsg + ", sign=" + sign + ", trxCode=" + trxCode
                + ", version=" + version + "]";
    }
